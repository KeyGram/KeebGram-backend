...
router.get("/getUserNotifications", (req, res) => {
  ...
  db.query(sql, [account_id], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      return res.status(404).send("Error with account id");
    }
  });
});
...
router.post("/create", (req, res) => {
  ...
  const query = "INSERT INTO notifications (post_id, account_id, message) VALUES (?, ?, ?)";

  db.query(query, [post?.post_id, post?.account_id, message], (err, results) => {
    if (err) {
