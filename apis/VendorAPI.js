    ...
    (err, results) => {
      if (err) {
        ...
        if (err?.errno === 1062) {
          return res
            .status(500)
            .send("Vendor Application already being processed");
        } else {
...
router.post("/delete", (req, res) => {
  ...
  const query =
    "DELETE FROM vendor_applications WHERE application_id = ?; DELETE FROM addresses WHERE address_id = ?";

  db.query(
    query,
    [application?.application_id, application?.address_id],
    (err, result) => {
      if (err) {
      ...
      const affectedRows = result.reduce(
...
router.get("/getVendorByAccountId", (req, res) => {
  ...
  db.query(query, [account_id], (err, results) => {
    if (err) {
      console.error(err);
      return res.status(500).send("(VendorAPI)Error fetching vendor data");
    }

    if (results.length === 0) {
      return res.status(404).send("(VendorAPI)Vendor not found");
    }
